...
id_dict <- getURL("https://raw.githubusercontent.com/arifpras/Fantasy-Premier-League/master/data/2021-22/id_dict.csv")

db00a <- read.csv(text = id_dict)
```
...
db00h <- db00f %>%
  ...
  filter(
    !team %in% relegated #not including the relegated teams
  )
```
...
```{r}
(gk_choice <- db03d %>% filter(id_gk == 11))
```
...
```{r}
(def_choice <- db04d %>% filter(id_def == 2049))
```
...
```{r}
(mid_choice <- db05d %>% filter(id_mid == 265))
```
...
```{r}
(fwd_choice <- db06d %>% filter(id_fwd == 140))
```
...
(belutlistrik_squad <- (gk_choice %>% rename("id" = id_gk)) %>%
   ...
   relocate(id, formid, Understat_Name)
 )
```
...
(belutlistrik_options <- belutlistrik_squad %>% 
   ...
   relocate(id, formid, form352, form442, Understat_Name) 
)
```
...
```{r}
(
  belutlistrik_form <- belutlistrik_options %>%
...
```{r}
(
  belutlistrik <- ggplot(belutlistrik_form) +
    annotate_pitch(
      colour = "white",
      fill   = "#e0e2db",
      limits = FALSE
    ...
    geom_point(
      aes(x = x, y = 100 - y, fill = factor(form442)),
      shape = 21,
      size = 5
    ) +
