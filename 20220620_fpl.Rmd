...
```{r}
(gk_choice <- db03d %>% filter(id_gk == 48) %>% mutate("position" = "GK"))
```
...
```{r}
(def_choice <- db04d %>% filter(id_def == 13289) %>% mutate("position" = "DEF"))
```
...
```{r}
(mid_choice <- db05d %>% filter(id_mid == 533) %>% mutate("position" = "MID"))
```
...
```{r}
(fwd_choice <- db06d %>% filter(id_fwd == 575) %>% mutate("position" = "FWD"))
```
