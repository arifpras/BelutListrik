...
Let's say the budget for the goalkeeper position is 9
...
```{r mid03}
tic("Time needed:") # 196.508 sec elapsed
(
  mid03 <- mid02 %>%
    # na.omit() %>%
    as_tibble() %>%
...
```{r blfinal}
(
  belutlistrik <- ggplot(belutlistrik_451) + #change
    annotate_pitch(
      colour = "white",
      fill   = "#e0e2db",
      limits = FALSE
    ...
    geom_point(
      aes(x = x, y = 100 - y, fill = factor(form451)), #change
      shape = 21,
      size = 8
    ) +
